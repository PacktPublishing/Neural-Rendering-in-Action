...
def make_camera(eye, target=(0, 0, 0), up=(0, 0, 1), width=240, height=180,
                fov=40.0):
    """Pinhole camera: one ray per pixel. Returns (origins, dirs, shape),
    ready to feed straight into sphere_trace."""
    eye = np.asarray(eye, np.float32); target = np.asarray(target, np.float32)
    # orthonormal camera basis (look-at): forward, then right, then true up
    fwd = target - eye; fwd /= np.linalg.norm(fwd)
    right = np.cross(fwd, up); right /= np.linalg.norm(right)
    upv = np.cross(right, fwd)                        # re-orthogonalize up
    a = np.tan(np.radians(fov) / 2)                  # half-height of image plane at unit depth
    xs = np.linspace(-a, a, width) * width / height  # * aspect ratio -> square pixels
    ys = np.linspace(a, -a, height)                  # top row first (y points down in image)
    X, Y = np.meshgrid(xs, ys)
    # ray dir per pixel: offset the forward vector within the image plane, normalize
    dirs = (X[..., None] * right + Y[..., None] * upv + fwd).reshape(-1, 3)
    dirs /= np.linalg.norm(dirs, axis=1, keepdims=True)
    origins = np.broadcast_to(eye, dirs.shape).astype(np.float32)   # all rays start at the eye
