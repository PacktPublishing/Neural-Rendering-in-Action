...
// Must match the default Network specialization in shaders/nrc/network.slang.
// NrcFeatureEncoder produces its 64 inputs in shaders/nrc/features.slang.
...
struct MlpLayout {
    ...
    // Per-layer bias sizes (always outputs * elementBytes).
    ...
    uint32_t elementBytes = kNrcElementBytes;
    VkComponentTypeKHR componentType = VK_COMPONENT_TYPE_FLOAT16_KHR;
};
...
// Query each precision independently: optimal layouts are opaque.
// Returns the complete layout, or throws on failure.
MlpLayout computeMlpLayout(
    VkDevice device, VkCooperativeVectorMatrixLayoutNV layout,
    VkComponentTypeKHR componentType = VK_COMPONENT_TYPE_FLOAT16_KHR);
