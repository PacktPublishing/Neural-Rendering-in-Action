...
static size_t queryMatrixSize(VkDevice device,
                              ...
                              VkComponentTypeKHR componentType, uint32_t elementBytes) {
    if (layout == VK_COOPERATIVE_VECTOR_MATRIX_LAYOUT_ROW_MAJOR_NV ||
        ...
        return static_cast<size_t>(rows) * cols * elementBytes;
    }
...
MlpLayout computeMlpLayout(VkDevice device, VkCooperativeVectorMatrixLayoutNV layout,
                           VkComponentTypeKHR componentType) {
    if(componentType != VK_COMPONENT_TYPE_FLOAT16_KHR
       && componentType != VK_COMPONENT_TYPE_FLOAT32_KHR)
        throw std::invalid_argument("nrc: unsupported MLP component type");
    MlpLayout ml;
    ml.componentType = componentType;
    ml.elementBytes = componentType == VK_COMPONENT_TYPE_FLOAT32_KHR ? 4u : 2u;
    ...
    for (int i = 0; i < kNrcNumLayers; ++i) {
        ...
        const size_t wBytes = queryMatrixSize(device, layout, ml.rows[i], ml.cols[i],
                                              ml.componentType, ml.elementBytes);
        ...
        ml.biasSize[i]   = ml.rows[i] * ml.elementBytes;
        offset           = alignUp(offset, kMatrixAlignment);
