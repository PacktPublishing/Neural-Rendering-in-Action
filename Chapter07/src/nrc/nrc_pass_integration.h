...
// Completion token of the submission containing the upcoming NRC commands.
// The application must signal this timeline value after all frame commands.
// Readbacks poll completed values without waiting for the GPU each frame.
void nrc_pass_set_frame_completion(VkSemaphore semaphore, uint64_t signalValue);
void nrc_pass_set_firefly_clamp(float threshold);
