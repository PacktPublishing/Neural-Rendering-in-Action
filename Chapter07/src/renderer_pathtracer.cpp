...
static void updateNrcFrameState(Resources& resources, VkExtent2D size, uint32_t numSamples, uint32_t totalSamplesAccumulated, bool firstFrame, float fireflyClamp)
{
  const auto completion = resources.app->getFrameSignalSemaphore();
  nrc_pass_set_frame_completion(completion.semaphore, completion.value);
  nrc_pass_set_firefly_clamp(fireflyClamp);
...
void PathTracer::registerParameters(nvutils::ParameterRegistry* paramReg)
{
  ...
  paramReg->add({"ptUseSER", "PathTracer: Enable shader execution reorder"}, &m_useSER);
  paramReg->add({"nrcLeftView", "NRC left view [PathTracer:0, NRC:1, CacheDebug:2]"}, &m_pushConst.nrcLeftViewMode);
  paramReg->add({"nrcRightView", "NRC right view [PathTracer:0, NRC:1, CacheDebug:2]"}, &m_pushConst.nrcRightViewMode);
...
void PathTracer::renderRayQuery(VkCommandBuffer cmd, VkExtent2D renderingSize, Resources& resources)
{
  ...
  if(nrcFrameEnabled)
  {
    ...
    updateNrcFrameState(resources, renderingSize, uint32_t(m_pushConst.numSamples), uint32_t(m_pushConst.totalSamples),
                        firstFrameOrReset, m_pushConst.fireflyClampThreshold);
...
void PathTracer::setupPushConstant(VkCommandBuffer cmd, Resources& resources)
{
...
#endif
  VkExtent2D nrcRenderSize = resources.gBuffers.getSize();
#if defined(USE_DLSS)
  if(useDlss) nrcRenderSize = m_dlss->getRenderSize();
#endif
#if defined(USE_OPTIX_DENOISER)
  if(useOptixDenoiser && m_optix->isUpscaleMode()) nrcRenderSize = m_optix->getRenderSize();
#endif
  m_pushConst.renderingSize = {nrcRenderSize.width, nrcRenderSize.height};
  ...
  // NRC normalizes feature positions into the scene AABB before frequency
  // encoding them. Outside this finite cache domain, continue path tracing.
  {
    glm::vec3 sceneMin(0.0f);
    glm::vec3 sceneInvExtent(1.0f);
    if(resources.getScene() && resources.getScene()->valid())
    {
      const nvutils::Bbox bbox   = resources.getScene()->getSceneBounds();
      const glm::vec3 extent = bbox.max() - bbox.min();
      // Padding handles degenerate axes and roundoff at the scene bounds.
      const float padding = std::max(1e-4f, std::max({extent.x, extent.y, extent.z}) * 1e-4f);
      sceneMin = bbox.min() - glm::vec3(padding);
      sceneInvExtent = glm::vec3(1.0f) / (extent + glm::vec3(2.0f * padding));
    }
    m_pushConst.nrcSceneMin       = sceneMin;
    m_pushConst.nrcSceneInvExtent = sceneInvExtent;
  }
